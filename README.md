...
- **Session management** — active JWT sessions are tracked in `UserSession` (plus live online-user and guest counts, taken from open SSE streams: `/notification/stream` for logged-in users, `/presence/guest-stream` for guests, pushed to admins on `/session/stream`), searchable per user and force-logout-able individually or globally via `UserSessionController` (`/session`)
...
│                        Analytics, Report)
├── service/             Business logic interfaces (incl. UserSessionService,
│                        CvGenerationService)
